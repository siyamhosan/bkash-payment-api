...
export class BkashGateway {
  ...
  public createPayment = async (
    ...
    return await post<IBkashCreatePaymentResponse>(
  ...
  public executePayment = async (
    ...
    try {
      const headers = await this.createExecuteHeader()
   ...
   * ```
   */
  public queryPayment = async (
    paymentID: string
  ): Promise<IBkashQueryPaymentResponse> => {
    const headers = await this.createTokenHeader()
    return await get<IBkashQueryPaymentResponse>(
   ...
   * ```
   */
  public searchTransaction = async (
    trxID: string
  ): Promise<IBkashSearchTransactionResponse> => {
  ...
  private createTokenHeader = async (isBearer?: boolean): Promise<IHeaders> => {
    const token = await this.getToken()
    isBearer = isBearer ?? true
    return {
  ...
  private createExecuteHeader = async () => {
    const token = await this.getToken()
    return {
      Authorization: token,
      'x-app-key': this.key,
  ...
  private getToken = async (): Promise<string> => {
    if (!this.token) {
      const { id_token, refresh_token, msg, status } =
    ...
    const { id_token, refresh_token, msg, status } = await this.newToken(
      this.refreshToken
    )
